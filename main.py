...
def main():
    ...
    print('счёт через матрицу:', most_freq(matrix, words), least_freq(matrix, words),
    ...
    print('счёт через словарь:', 'самое частое слово:'.format(most_common(inverted_dict)),
          'редких слов очень много, все они встречаются по одному разу: {}'.format(less_common(inverted_dict)),
