...
def create(corpus):
    ...
    questions_prep, questions, answers = [], [], []
    ...
    for num, val in tqdm(enumerate(corpus)):
        ...
        try:
            ...
            questions_prep.append(preprocess(json.loads(corpus[num])['question']))
            questions.append(json.loads(corpus[num])['question'])
            answers.append(answer)
        except:
            continue

    tips_data['question'], tips_data['non_prep_question'], tips_data['answer'] = questions_prep, questions, answers
    return tips_data
